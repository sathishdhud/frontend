...
const Admin: React.FC = () => {
  ...
  // Modal states - replacing the previous notification states
  ...
  const showOperationsModal = (
    ...
    showModal(title, message, type, action, showConfirm, showCancel, confirmBtnText, cancelBtnText);
  };
  ...
  const handleUserFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showModal('Error', `Failed to ${editingUser ? 'update' : 'create'} user. ${error.response?.data?.message || 'Please try again.'}`, 'error');
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      switch (type) {
        case 'Room':
          ...
          showModal('Success', `Room ${editingRoom ? 'updated' : 'created'} successfully!`, 'success');
        ...
        case 'Account Head':
          if (editingAccountHead) {
            ...
            // Generate a unique ID for new account heads if not provided
            const accountData = {
              ...formData,
              accHeadId: formData.accHeadId || `ACC${Date.now()}`
            };
            response = await masterDataApi.createAccountHead(accountData);
          }
          break;
        case 'Room Type':
          if (editingRoomType) {
      ...
      showModal('Success', `${editingTax || editingAccountHead || editingRoomType || editingPaymentMode || editingPlanType || editingCompany || editingNationality || editingRefMode || editingArrivalMode || editingReservationSource || editingSettlementType || editingRoom ? 'Updated' : 'Created'} ${type} successfully!`, 'success');
      ...
      switch (type) {
        case 'Tax':
          ...
          break;
        case 'Account Head':
          setAccountHeadForm({ accHeadId: '', name: '', companyName: '', chequeNumber: '', date: '' });
      ...
      showModal('Error', `Failed to ${editingTax || editingAccountHead || editingRoomType || editingPaymentMode || editingPlanType || editingCompany || editingNationality || editingRefMode || editingArrivalMode || editingReservationSource || editingSettlementType || editingRoom ? 'update' : 'create'} ${type}. ${error.response?.data?.message || 'Please try again.'}`, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteMasterData = async (type: string, id: string) => {
    showModal(
      ...
      () => {
        // This is the confirm action
        performMasterDataDeletion(type, id);
      },
      true,
      true,
      'Delete',
  ...
  const performMasterDataDeletion = async (type: string, id: string) => {
    try {
      switch (type) {
        case 'Tax':
      ...
      showModal('Success', `${type} deleted successfully!`, 'success');
      fetchMasterData();
      fetchRooms();
      ...
      showModal('Error', `Failed to delete ${type}. ${errorMessage}`, 'error');
    }
  };
  ...
  const handleAuditDateChange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (auditDateForm.confirmation !== 'YES') {
        showOperationsModal('Confirmation Required', 'Please confirm by typing "YES" to proceed with audit date change', 'warning');
      ...
      if (response.data.success) {
        showOperationsModal('Success', response.data.message || 'Audit date change processed successfully!', 'success');
        // Reset form
        setAuditDateForm({ confirmation: '' });
        // Refresh data if needed
        fetchMasterData();
      } else {
        showOperationsModal('Error', response.data.message || 'Failed to process audit date change', 'error');
      }
    } catch (error: any) {
      ...
      showOperationsModal('Error', `Error: ${errorMessage}`, 'error');
    } finally {
      setOperationsLoading(false);
    }
  };
  ...
  const handleShiftChange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        showOperationsModal('Success', response.data.message || 'Shift change processed successfully!', 'success');
        ...
        showOperationsModal('Error', response.data.message || 'Failed to process shift change', 'error');
      }
    } catch (error: any) {
  ...
  const handleShiftClose = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        showOperationsModal('Success', response.data.message || 'Shift closed successfully!', 'success');
        // Reset form to default values
        setShiftCloseForm({ 
          balance: 0,
          openingBalance: 0,
        ...
        showOperationsModal('Error', response.data.message || 'Failed to close shift', 'error');
      }
    } catch (error: any) {
            ...
            <>
              {renderForm(
                ...
                (e) => {
                    ...
                    const formData = { ...accountHeadForm };
                    
                    // If we're editing, remove the accHeadId from the data to avoid conflicts
                    if (editingAccountHead) {
                      const { accHeadId, ...dataWithoutId } = formData;
                      handleMasterDataFormSubmit(e, 'Account Head', dataWithoutId);
                    } else {
                      // For new account heads, ensure we have a valid ID
                      if (!formData.accHeadId) {
                        formData.accHeadId = `ACC${Date.now()}`;
                      }
                      handleMasterDataFormSubmit(e, 'Account Head', formData);
                    }
                  },
                [
                  { label: 'Account Head ID', name: 'accHeadId', required: true },
              ...
              {renderMasterDataList('Account Heads', accountHeads, [
                { header: 'Account Head ID', key: 'accHeadId', render: (item: AccountHead) => item.accHeadId || '-' },
